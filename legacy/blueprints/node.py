...
def node_info():
    """
    通过节点ID从本地PostgreSQL数据库获取节点详细信息
    """
    # 1. 获取请求中的 NodeId
    ...
    if current_user.role != 'manager':
        conditions.append("nodes.user_id = %s")
        params.append(current_user.id)

    # 添加节点ID过滤
    conditions.append("nodes.id = %s")
...
def approve_routes():
    ...
    with DB() as cursor:
        cursor.execute("SELECT route FROM users WHERE id = %s ",(current_user.id,))
