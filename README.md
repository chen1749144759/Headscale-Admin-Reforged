...
- 管理员创建或重置账户时可生成随机初始密码，提交成功后仅在当前页面回显一次；平台不保存可逆明文，关闭弹窗后只能重新重置。普通用户可在 ScaleTail Windows 客户端中设置自己的新密码并自动继续连接。
- 修改密码不能复用当前密码和最近四个历史密码；更新会撤销旧管理会话，并要求节点在新的控制会话中重新完成账户证明。
- 密码自最后修改起最长 90 天有效。Windows 会在首次或到期登录时引导改密；Linux 托管节点从第 83 天起通过 Noise 主动生成并轮换密码，随后自动重新认证。
- ScaleTail Linux 交互登录使用 `--username` 并在终端内隐藏读取密码；自动化使用权限为 `0600` 的 `--password-file` 和 systemd 周期任务。平台无法读取 Linux 自动生成的当前密码，密码也不能放进命令行、环境变量、镜像层或 Git。
